<?php
	...
	$pending_email_filename 	= './guzzle/pending/'.$section_number.'_Pending_emails_' . $date_append . '.csv';
	$processed_email_filename 	= './guzzle/processed/'.$section_number.'_Processed_list_' . $date_append . '.csv';
	$failed_email_filename 		= './guzzle/failed/'.$section_number.'_Failed_list_' . $date_append . '.csv'; 
	$divisor = 45 ; // number of emails you want per section 
	$all_emails = array();
	$section_emails;
	$sections;
...
//	If there's $pending_emails , recheck by Circpro 
	if(!empty($pending_emails)){
		global $final_email_list, $processed_emails, $processed_emails_2;
